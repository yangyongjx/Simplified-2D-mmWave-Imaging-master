...
function sarImage = reconstructSARimageMatchedFilterSimplified(sarData, matchedFilter, xStepM, yStepM, xSizeT, ySizeT)
    ...
    xRangeT = xStepM * (-(xPointT - 1) / 2:(xPointT - 1) / 2); % xStepM is in mm      0.5mm
    yRangeT = yStepM * (-(yPointT - 1) / 2:(yPointT - 1) / 2); % yStepM is in mm      2mm


    %% Crop the Image for Related Region
    indXpartT = xRangeT > (-xSizeT / 2) & xRangeT < (xSizeT / 2);
    indYpartT = yRangeT > (-ySizeT / 2) & yRangeT < (ySizeT / 2);
